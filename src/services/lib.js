...
function calculatePoint45(lat1, lon1, bearing) {
    ...
    const lat1_45 = Math.asin(Math.sin(lat) * Math.cos(angularDistance) + Math.cos(lat) * Math.sin(angularDistance) * Math.cos(bearing + (45 * Math.PI) / 180));
    const lon1_45 = lon + Math.atan2(Math.sin(bearing + (45 * Math.PI) / 180) * Math.sin(angularDistance) * Math.cos(lat), Math.cos(angularDistance) - Math.sin(lat) * Math.sin(lat2_135));

    // Convert back to degrees
    const newLat_45 = (lat2_135 * 180) / Math.PI;
    const newLon_45 = (lon2_135 * 180) / Math.PI;
...
//test
console.log(lib.calculateBBox(39.17539978, -76.66829681, 26.2471008301, -80.1110992432));
